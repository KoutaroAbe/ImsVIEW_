...
//		IJ.showStatus("ImsOpener("+ fileList.size() +" files): ");
	...
	public void open() {
		...
		if( asStack==true ) {
			if( fileList.size()==1 && fileList.get(0).isDirectory() ) { // when selected one directory
				...
				imp = openAsStack( childList, getFileInfo( fileList.get(0) ) );
			} else
				// Open multiple files as a Stack
				imp = openAsStack( fileList, getFileInfo( fileList.get(0).getParentFile() ) );
		} else
			// Open Slice(s) 
			for( i=0; i<fileList.size(); i++ )
				imp = openAsSlice( getFileInfo( fileList.get(i) ) );
	}
	...
	ImagePlus openAsSlice( FileInfo fi ){
		...
		if( imp==null )
			imp = new LimasOpener( filepath ).open(true);
		if( imp==null && isIms(f) )
			imp = new FileOpener(fi).open(true);
		if( imp==null ) {
			IJ.log( "Skipped '" + fi.directory+" / " + fi.fileName + "' (invalid file)." );
			return null;
		}
		imp.setFileInfo(fi);
		return imp;
...
//			IJ.log( "openAsStack["+i+"] "+fi.directory+" / "+fi.fileName+": file?"+f.isFile()+" dir?"+f.isDirectory()+" hidden?"+f.isHidden() );
			if( f.isFile() ) {
				imp = new HDF32Opener( f ).open(false);
				if( imp==null ) {
					imp = new LimasOpener( f ).open(false);
				}
				if( imp==null && isIms(f) ){
					imp = new FileOpener(fi).open(false);
				}
				if( imp==null ) {
					IJ.log( "Skipped '" + fi.directory + " / "+ fi.fileName + "' (invalid file). continue ..." );
					skipped++;
					continue;
				}
				if (stack == null)
